...
def overlay_image_onto_background(image, mask, bbox, background, opaque_f=1.0):
    ...
    orig_img_roi = roi_image.copy()

    roi_image[mask] = image[mask]
    out_image[bbox[1] : bbox[3], bbox[0] : bbox[2]] = opaque_f * roi_image + (1 - opaque_f) * orig_img_roi
...
class Renderer:
    ...
    def render_mesh(self, vertices, background=None, colors=[0.8, 0.8, 0.8], VI=50, opaque_f=1.0):
        ...
        image = overlay_image_onto_background(image, mask, self.bboxes, background.copy(), opaque_f=opaque_f)
        self.reset_bbox()
        return image
