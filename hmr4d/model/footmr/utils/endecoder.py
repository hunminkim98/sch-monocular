...
class EnDecoder(nn.Module):
    ...
    def normalize_ankle_pose_r6d(self, rel_ankle_r6d):
        ...
        rel_ankle_r6d = (rel_ankle_r6d - self.mean[6 * 6 : 8 * 6]) / self.std[
            6 * 6 : 8 * 6
        ]  # (B, L, C)
        return rel_ankle_r6d

    def normalize_foot_pose_r6d(self, rel_foot_r6d):
        B, L = rel_foot_r6d.shape[:2]
        rel_foot_r6d = rel_foot_r6d.reshape(B, L, -1)
        rel_foot_r6d = (rel_foot_r6d - self.mean[9 * 6 : 11 * 6]) / self.std[
            9 * 6 : 11 * 6
        ]  # (B, L, C)
        return rel_foot_r6d
