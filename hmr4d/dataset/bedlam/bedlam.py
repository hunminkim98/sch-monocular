...
from hmr4d.dataset.bedlam.utils import mid2featname
from hmr4d.utils.geo_transform import compute_cam_angvel
from hmr4d.utils.geo.hmr_global import get_T_w2c_from_wcparams, get_R_c2gv
...
class BedlamDatasetV2(ImgfeatMotionDatasetBase):
    ...
    def _load_dataset(self):
        ...
        if self.random1024:  # 디버그 시 빠르게 불러옵니다.
            try:
                ...
                Log.info(f"[{self.dataset_name}] Not found, saving 1024 samples to disk ...")
        ...
        Log.info(
            f"[{self.dataset_name}] {len(self.seqs)} raw sequences. Elapsed: {Log.time() - tic:.2f}s"
        )

    def _get_idx2meta(self):
        # sum_frame = sum([e-s for s, e in self.mid_to_valid_range.values()])
        self.idx2meta = list(self.mid_to_valid_range.keys())
        Log.info(f"[{self.dataset_name}] {len(self.idx2meta)} train samples. ")
    ...
    def _load_data(self, idx):
        ...
        data["vid"] = mid
        data["kp2d"] = torch.zeros((end - start), 23, 3)
    ...
    def _process_data(self, data, idx):
        ...
        video_path = os.path.join("inputs/BEDLAM/hmr4d_support/videos", video_path + ".mp4")
        return_data = {
            "meta": {
                "data_name": "bedlam",
                "idx": idx,
                "width_height": (data["img_wh"][0], data["img_wh"][1]),
                "vid": data["vid"],
...
MainStore.store(
    name="v2_random1024", node=builds(BedlamDatasetV2, random1024=True), group=group_name
)
